...
static tft_st7735_status_t stm32_spi_write(const uint8_t *data, uint32_t len, void *ctx) {
    tft_st7735_port_stm32_ctx_t *c = (tft_st7735_port_stm32_ctx_t *)ctx;
    HAL_StatusTypeDef err = HAL_SPI_Transmit(c->hspi, (uint8_t *)data, len, HAL_MAX_DELAY);
    ...
    uint32_t timeout_ms, void *ctx)
{
    ...
    if (tx_len > 0) {
        HAL_StatusTypeDef err = HAL_SPI_Transmit(c->hspi, (uint8_t *)tx_data, tx_len, timeout_ms);
        if (err != HAL_OK) return TFT_ST7735_ERROR;
    }

    if (rx_len > 0) {
        HAL_StatusTypeDef err = HAL_SPI_Receive(c->hspi, rx_data, rx_len, timeout_ms);
        if (err != HAL_OK) return TFT_ST7735_ERROR;
    }
...
static void stm32_set_dc(bool is_data, void *ctx) {
    tft_st7735_port_stm32_ctx_t *c = (tft_st7735_port_stm32_ctx_t *)ctx;
    HAL_GPIO_WritePin(c->dc.port, c->dc.pin, is_data ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

static void stm32_set_reset(bool active, void *ctx) {
    tft_st7735_port_stm32_ctx_t *c = (tft_st7735_port_stm32_ctx_t *)ctx;
    HAL_GPIO_WritePin(c->reset.port, c->reset.pin, active ? GPIO_PIN_RESET : GPIO_PIN_SET);
}

static void stm32_set_cs(bool active, void *ctx) {
    tft_st7735_port_stm32_ctx_t *c = (tft_st7735_port_stm32_ctx_t *)ctx;
    HAL_GPIO_WritePin(c->cs.port, c->cs.pin, active ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
